...
class vgg16:
    def __init__(self, imgs, weights=None, sess=None, c1=None, c2=None, c3=None):
        self.imgs = imgs
    ...
    def convlayers(self):
        ...
        with tf.name_scope('preprocess') as scope:
            mean = tf.constant([self.c1, self.c2, self.c3], dtype=tf.float32, shape=[1, 1, 1, 3], name='img_mean')
        ...
        with tf.name_scope('conv2_1') as scope:
            ...
            biases = tf.Variable(tf.constant(0, shape=[128], dtype=tf.float32), trainable=True, name='biases')
        ...
        with tf.name_scope('conv3_3') as scope:
            kernel = tf.Variable(tf.truncated_normal([3,3,256,256], dtype=tf.float32, stddev=1e-1), name='weights')
            biases = tf.Variable(tf.constant(1e-2, shape=[256], dtype=tf.float32), trainable=True, name = 'biases')
        ...
        with tf.name_scope('conv4_1') as scope:
            ...
            biases = tf.Variable(tf.constant(1e-2, shape=[512], dtype=tf.float32), trainable=True, name = 'biases')
    ...
    def fc_layers(self):
        ...
        with tf.name_scope('fc1') as scope:
            ...
            fc1b = tf.Variable(tf.constant(0, shape=[4096], dtype=tf.float32), trainable=True, name = 'biases')
        ...
        with tf.name_scope('fc2') as scope:
            ...
            fc2b = tf.Variable(tf.constant(1e-1, shape=[4096], dtype=tf.float32), trainable=True, name='biases')
        ...
        with tf.name_scope('fc3') as scope:
            ...
            fc3b = tf.Variable(tf.constant(1e-2, shape=[1000], dtype=tf.float32), name='biases')
...
if __name__ == '__main__':
    ...
    img1 = imread('cat.png', mode='RGB')
    img1 = imresize(img1, (224, 224))
    c1, c2, c3 = img1[:, :, 0].mean(), img1[:, :, 1].mean(), img1[:, :, 2].mean()
    vgg = vgg16(imgs, 'vgg16_weights.npz', sess, c1, c2, c3)


    prob = sess.run(vgg.probs, feed_dict={vgg.imgs: [img1]})[0]
    preds = (np.argsort(prob)[::-1])[0:4]
    for p in preds:
        print 'Possible class: ' + str(class_names[p]) + ' and corresponding probability: ' + str(prob[p])
        print '--------------------------------------------------------------------------'
